...
class StrategyPlanner:
    ...
    async def plan(self, payload: StrategyPlannerInput) -> StrategyPlan:
        ...
        plan = _enforce_risk_route(payload, plan)
...
def _enforce_risk_route(
    payload: StrategyPlannerInput,
    plan: StrategyPlan,
) -> StrategyPlan:
    """Prevent ordinary dialogue strategies when risk routing requires safety handling."""

    if payload.risk.route == RiskRoute.NORMAL:
        return plan
    if plan.primary_strategy == StrategyType.SAFETY_CHECK:
        return plan
    return StrategyPlan(
        conversation_phase=ConversationPhase.SAFETY_CHECK,
        primary_strategy=StrategyType.SAFETY_CHECK,
        objective="prioritize checking and handling the current safety risk",
        reason=f"{plan.reason}; overridden because risk route is {payload.risk.route.value}",
        avoid=sorted(
            set(
                plan.avoid
                + ["continue ordinary dialogue", "ignore safety signals", "diagnosis", "medication advice"]
            )
        ),
        expected_signals=plan.expected_signals or ["user safety status is clarified"],
        switch_conditions=plan.switch_conditions or ["risk clears", "risk escalates"],
    )
