...
class RiskAgent:
    ...
    def _fallback_result(self, payload: RiskInput, error: Exception) -> RiskResult:
        ...
        return RiskResult(
            ...
            needs_clarification=False,
            route=RiskRoute.NORMAL,
            reason_codes=[reason, "keyword_only_fallback"],
            confidence=0.3,
        )
...
def _detected_categories(payload: RiskInput) -> list[str]:
    """Detect high-signal safety categories from the current user turn only."""

    if payload.current_message.role != MessageRole.USER:
        return []
    text = payload.current_message.content.lower()
    categories: list[str] = []
    self_harm = _contains_unnegated_term(text, _SELF_HARM_TERMS)
    harm_others = _contains_unnegated_term(text, _HARM_OTHERS_TERMS)
...
def _contains_unnegated_term(text: str, terms: tuple[str, ...]) -> bool:
    """Return true when a risk keyword is not locally negated."""

    negations = ("没有", "没", "不", "不会", "不是", "无", "no", "not", "never")
    for term in terms:
