...
_PENDING_MEMORY_REWRITE = (
    "我这里有一条与这段话相关但尚待确认的记录。在你确认前，我不会把它当作事实；"
    "你愿意确认或补充一下吗？"
)
...
class FakeOutputGuard:
    ...
    async def review(self, payload: GuardInput) -> GuardResult:
        """Apply deterministic pre-checks used by the model-backed guard."""
...
class OutputGuard:
    ...
    async def review(self, payload: GuardInput) -> GuardResult:
        """Return a conservative typed guard decision without modifying input."""
...
def _rule_review(payload: GuardInput) -> GuardResult | None:
    ...
    content_violations = _rule_violations(text)
    memory_violations = _memory_violations(payload)
    ...
    if content_violations:
        return GuardResult(
            ...
            violations=sorted(set(content_violations)),
            rewritten_response=_FIXED_SAFE_RESPONSE,
        )
    if memory_violations:
        violations = sorted(set(memory_violations))
        if payload.context.memory_query_intent != MemoryQueryIntent.NONE:
            rewritten_response = build_memory_query_draft(payload.context).text
        elif "pending_memory_presented_as_confirmed" in memory_violations:
            rewritten_response = _PENDING_MEMORY_REWRITE
        else:
            return GuardResult(
                decision=GuardDecision.BLOCK,
...
def _memory_violations(payload: GuardInput) -> list[str]:
    """Validate memory IDs and claims against owned retrieved context."""

    active_memories = payload.context.memories.active_memories or (
        payload.context.memories.semantic_memories
        + payload.context.memories.episodic_memories
    )
    pending_memories = payload.context.memories.pending_confirmation_memories
    known_ids = {
        str(memory.id) for memory in active_memories + pending_memories
    }
    violations: list[str] = []
    if any(
        ...
        violations.append("inappropriate_memory_use")

    known_knowledge_ids = {
        evidence.chunk_id for evidence in payload.context.knowledge.evidences
    }
    if any(
        knowledge_id not in known_knowledge_ids
        for knowledge_id in payload.draft.referenced_knowledge_ids
    ):
        violations.append("inappropriate_knowledge_use")

    intent = payload.context.memory_query_intent
    relevant_memories = (
        select_relevant_memories(
            active_memories + pending_memories,
            payload.context.current_user_message,
            intent,
        )
        if intent != MemoryQueryIntent.NONE
        else []
    )
    if relevant_memories and contains_false_memory_denial(payload.draft.text):
        violations.append("memory_query_false_denial")

    pending_ids = {str(memory.id) for memory in pending_memories}
    referenced_pending_ids = pending_ids.intersection(
        payload.draft.referenced_memory_ids
    )
    if referenced_pending_ids and not _marks_memory_as_unconfirmed(
        payload.draft.text
    ):
        violations.append("pending_memory_presented_as_confirmed")
    return violations


def _marks_memory_as_unconfirmed(text: str) -> bool:
    """Return whether the response clearly labels pending content as uncertain."""

    markers = ("待确认", "尚未确认", "未确认", "可能", "是不是", "是否", "请你确认")
...
__all__ = ["FakeOutputGuard", "OutputGuard"]
