...
async def run_conversation(
    *,
    ...
    for index, text in enumerate(turns, 1):
        ...
        await print_storage_snapshot(session_factory, user_id, session_id)
        print()
    ...
    await print_storage_snapshot(
        session_factory, user_id, session_id, include_memories=True
    )
    ...
    async with session_factory() as session:
        ...
        counts = {
            ...
            "sessions": await session.scalar(
                select(func.count()).select_from(SessionModel).where(*owner_scope)
            ),
