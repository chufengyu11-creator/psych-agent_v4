...
class AsyncClosable(Protocol):
    """Resource that can drain or close itself asynchronously."""
...
class ApplicationRuntime:
    ...
    feedback_evaluator: FeedbackAnalyzer | None = None
    task_queue: TaskQueue | None = None
    deep_state_queue: AsyncClosable | None = None
    session_closer: object | None = None
    _closed: bool = field(default=False, init=False)
    ...
    async def aclose(self) -> None:
        ...
        self._closed = True
        try:
            if self.deep_state_queue is not None:
                await self.deep_state_queue.aclose()
        finally:
            try:
                if isinstance(self.task_queue, AsyncClosable):
