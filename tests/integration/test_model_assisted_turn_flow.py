...
async def test_model_assisted_pipeline_runs_one_turn_with_structured_subagents() -> None:
    ...
    llm_client = QueueLLMClient(
        [
            ...
            """,
            _response_json("I hear that work feels stuck and heavy right now."),
            _guard_allow_json(),
        ]
    )
    ...
    assert len(llm_client.requests) == 5
...
def _response_json(text: str) -> str:
    """Return a DraftResponse JSON response for the response agent."""
...
def _guard_allow_json() -> str:
    """Return an allowing GuardResult JSON response."""
...
def _feedback_absent_json() -> str:
    """Return an absent-feedback model draft."""
...
async def test_model_assisted_pipeline_runs_work_stress_fixture_corpus() -> None:
    ...
    assert len(llm_client.requests) == 17
    assert all(request.response_format == "json_object" for request in llm_client.requests)
