...
async def test_list_active_returns_recently_updated_memories_first(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    """Active memory retrieval should prefer newer updates before truncation."""

    user_id = UserId("memory-order-user")
    async with session_factory() as session:
        older_source = await _create_source_message(session, user_id, "older")
        newer_source = await _create_source_message(session, user_id, "newer")
        repository = SqlAlchemyMemoryRepository(session)
        older = await repository.create(
            user_id,
            _candidate(older_source, content="Older preference."),
            _decision(MemoryOperation.CREATE),
        )
        newer = await repository.create(
            user_id,
            _candidate(newer_source, content="Newer preference."),
            _decision(MemoryOperation.CREATE),
        )
        assert older.memory_id is not None
        assert newer.memory_id is not None
        older_row = await session.get(LongTermMemoryModel, str(older.memory_id))
        newer_row = await session.get(LongTermMemoryModel, str(newer.memory_id))
        assert older_row is not None
        assert newer_row is not None
        older_row.updated_at = datetime(2026, 1, 1, tzinfo=UTC)
        newer_row.updated_at = older_row.updated_at + timedelta(days=1)
        await session.flush()

        active = await repository.list_active(user_id)

    assert [memory.id for memory in active] == [newer.memory_id, older.memory_id]
...
async def test_concurrent_duplicate_create_reinforces_existing_active(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    """A duplicate write must reinforce the existing row, never create a second."""

    user_id = UserId("memory-dedup-user")
    async with session_factory() as session:
        source_id = await _create_source_message(session, user_id, "dedup")
        candidate = _candidate(
            source_id,
    ...
    assert first.operation == MemoryOperation.CREATE
    assert second.applied is True
    assert second.operation == MemoryOperation.REINFORCE
    assert len(active) == 1
    assert row is not None
    assert row.reinforcement_count == 1


async def test_create_persists_content_hash_and_source_session(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    """Normalization columns must be written with every created memory."""

    user_id = UserId("memory-normalized-user")
    session_id = SessionId(f"session-{user_id}")
    async with session_factory() as session:
        source_id = await _create_source_message(session, user_id, "normalized")
        candidate = _candidate(
            source_id,
            content="User prefers one small step at a time.",
        )
    ...
    assert result.applied is True
    assert row is not None
    assert row.content_hash == sha256(
        "user prefers one small step at a time.".encode("utf-8")
    ).hexdigest()
    assert row.source_session_id == f"session-{user_id}"


async def test_confirm_pending_supersede_activates_replacement_and_retires_old_memory(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    """User confirmation must complete an otherwise pending replacement."""

    user_id = UserId("memory-confirm-supersede-user")
    async with session_factory() as session:
        old_source = await _create_source_message(session, user_id, "old")
        new_source = await _create_source_message(session, user_id, "new")
        repository = SqlAlchemyMemoryRepository(session)
        old_result = await repository.create(
            user_id,
            _candidate(old_source, content="小明今年40岁"),
            _decision(MemoryOperation.CREATE),
        )
        assert old_result.memory_id is not None
        pending_result = await repository.create(
            ...
            _decision(
                MemoryOperation.SUPERSEDE,
                target_memory_id=old_result.memory_id,
                requires_user_confirmation=True,
            ),
        )
        assert pending_result.memory_id is not None
        pending = await session.get(
            LongTermMemoryModel,
            str(pending_result.memory_id),
        )
        old = await session.get(LongTermMemoryModel, str(old_result.memory_id))
        assert pending is not None and pending.status == MEMORY_STATUS_PENDING_CONFIRMATION
        assert old is not None and old.status == MEMORY_STATUS_ACTIVE

        resolution = await repository.resolve_pending_confirmation(
            user_id,
            pending_result.memory_id,
            confirmed=True,
        )
    ...
    assert resolution.status == MEMORY_STATUS_ACTIVE
    assert resolution.superseded_memory_id == old_result.memory_id
    assert pending.status == MEMORY_STATUS_ACTIVE
    assert pending.user_confirmed is True
    assert old.status == MEMORY_STATUS_SUPERSEDED


async def test_reject_pending_memory_marks_it_deleted(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    """Rejected candidates must no longer appear as pending or active memory."""

    user_id = UserId("memory-reject-pending-user")
    async with session_factory() as session:
        source = await _create_source_message(session, user_id, "pending")
        repository = SqlAlchemyMemoryRepository(session)
        pending_result = await repository.create(
            user_id,
            _candidate(source),
        ...
        row = await session.get(LongTermMemoryModel, str(pending_result.memory_id))

    assert resolution is not None
    assert resolution.confirmed is False
    assert resolution.status == MEMORY_STATUS_DELETED
    assert row is not None and row.status == MEMORY_STATUS_DELETED
