    ...
    try:
        ...
        async with session_factory() as session:
            ...
            assert (
                await session.scalar(
                    select(SessionModel).where(
                        SessionModel.user_id == str(current.user_id),
                        SessionModel.session_id == str(current.session_id),
                    )
                )
                is None
            )
            ...
            assert (
                await session.scalar(
                    select(SessionModel).where(
                        SessionModel.user_id == str(unrelated.user_id),
                        SessionModel.session_id == str(unrelated.session_id),
                    )
                )
                is not None
            )
