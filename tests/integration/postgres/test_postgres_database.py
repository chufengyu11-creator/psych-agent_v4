...
async def test_postgres_repository_commit_and_round_trip(
    ...
    async with transactional_session(postgres_session_factory) as session:
        ...
        await session_repository.ensure_session(user_id, session_id)
        ...
        await SqlAlchemySummaryRepository(session).save_version(user_id, summary)
    ...
    async with postgres_session_factory() as session:
        ...
        assert await SqlAlchemySessionRepository(session).exists(user_id, session_id)
        recent = await SqlAlchemyMessageRepository(session).get_recent(
            user_id, session_id
        )
        current_state = await SqlAlchemyStateRepository(session).get_current(
            user_id, session_id
        )
        pending = await SqlAlchemyInterventionRepository(session).get_pending(
            user_id, session_id
        )
        ...
        state_row = await session.scalar(
            ...
            .join(SessionModel, SessionStateVersionModel.session_pk == SessionModel.id)
            .where(
    ...
    async with transactional_session(postgres_session_factory) as session:
        ...
        session_row = await session.scalar(
            select(SessionModel).where(
                SessionModel.user_id == str(postgres_case.user_id),
                SessionModel.session_id == str(postgres_case.session_id),
            )
        )
