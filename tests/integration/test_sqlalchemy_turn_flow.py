...
async def test_sqlalchemy_turn_flow_persists_core_rows(
    ...
    async with session_factory() as session:
        ...
        stored_session = await session.scalar(
            select(SessionModel).where(
                SessionModel.user_id == "db-user-1",
                SessionModel.session_id == "db-session-1",
            )
        )
        messages = list(
            (
    ...
    assert interventions[0].session_pk == stored_session.id
...
async def test_sqlalchemy_turn_flow_reads_previous_pending_intervention(
    ...
    async with session_factory() as session:
        ...
        intervention_records = await SqlAlchemyInterventionRepository(
            session
        ).list_for_session(UserId("db-user-2"), SessionId("db-session-2"))
