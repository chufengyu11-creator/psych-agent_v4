...
class StubStructuredClient:
    """Minimal structured client that records calls for response-agent tests."""
    ...
    async def generate_structured(
        ...
        *,
        ...
        """Return the configured structured result or raise the configured error."""
...
async def test_response_agent_calls_structured_client_and_returns_draft() -> None:
    """ResponseAgent should consume ResponseContext through structured output."""
    ...
    assert client.calls[0]["output_model"] == DraftResponse.__name__
    assert client.calls[0]["model_name"] == "response-model"
    assert client.calls[0]["metadata"] == {"agent": "response_agent"}
    assert "memories_json" in str(client.calls[0]["prompt"])
