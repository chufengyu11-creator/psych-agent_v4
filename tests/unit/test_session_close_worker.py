...
class RecordingSessionRepository:
    ...
    async def ensure_session(self, user_id: UserId, session_id: SessionId) -> None:
        """Accept the configured owned session."""

        assert (user_id, session_id) == (self.user_id, self.session_id)

    async def exists(self, user_id: UserId, session_id: SessionId) -> bool:
        """Return whether the configured session was requested."""

        return user_id == self.user_id and session_id == self.session_id

    async def assert_owned_by(self, user_id: UserId, session_id: SessionId) -> None:
        """Record and verify ownership."""

        self.assert_calls += 1
        assert (user_id, session_id) == (self.user_id, self.session_id)

    async def close(self, user_id: UserId, session_id: SessionId) -> None:
...
async def test_worker_assembles_complete_finalizer_input_and_returns_result() -> None:
    ...
    assistant_message = await messages.create_assistant_message(
        user_id, session_id, "A grounded response"
    )
    ...
    await states.save_version(user_id, final_state)
    ...
    await summaries.save_version(user_id, rolling_summary)
...
async def test_worker_policy_gates_model_backed_finalizer_candidate() -> None:
    ...
    worker = SessionCloseMemoryWorker(
        ...
        session_repository=sessions,
        finalizer=SessionFinalizer(WorkerFinalizerStructuredClient()),
        memory_policy=policy,
    )
