...
def test_session_columns_have_expected_types_and_defaults() -> None:
    ...
    assert table.c.id.primary_key
    assert isinstance(table.c.id.default, CallableColumnDefault)
    ...
    assert table.c.session_id.type.length == 64
...
def test_session_constraints_are_named_and_complete() -> None:
    ...
    assert unique_constraints == {
        "uq_sessions_user_id_session_id": ("user_id", "session_id")
    }
    assert set(checks) == {
        "ck_sessions_status",
...
def test_id_and_counter_default_responsibilities_are_explicit() -> None:
    """Internal session UUIDs and counters should retain model defaults."""
    ...
    assert isinstance(session_table.c.id.default, CallableColumnDefault)
    assert callable(session_table.c.id.default.arg)
