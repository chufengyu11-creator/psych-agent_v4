...
async def test_sqlalchemy_model_runtime_uses_formal_clients_and_model_agents() -> None:
    ...
    try:
        ...
        assert isinstance(runtime.response_agent, ResponseAgent)
        assert isinstance(runtime.output_guard, OutputGuard)
...
async def test_invalid_database_url_and_production_in_memory_fail_safely() -> None:
    ...
    with pytest.raises(RuntimeConfigurationError, match="production"):
        await build_application_runtime(
            _settings("in_memory", app_env="production")
        )
...
def test_settings_resolve_agent_specific_model_routes() -> None:
    """Each Agent override should resolve independently from legacy defaults."""

    settings = _settings(
        "sqlalchemy_model",
        llm_model_routes={
            "local-small": "https://small.test/v1",
            "local-medium": "https://medium.test/v1",
            "local-large": "https://large.test/v1",
        },
    ...
    assert settings.agent_model_names() == {
        "risk_agent": "local-small",
        "state_tracker": "local-medium",
        "feedback_evaluator": "local-small",
        "strategy_planner": "local-medium",
        "response_agent": "local-large",
        "output_guard": "local-small",
        "rolling_summarizer": "local-small",
        "session_finalizer": "local-medium",
        "memory_curator": "local-medium",
    }
    assert settings.required_llm_model_names() == (
        "local-small",
        "local-medium",
        "local-large",
    )
    routes = settings.resolved_llm_model_routes()
    assert routes["local-small"] == "https://small.test/v1"
    assert routes["local-medium"] == "https://medium.test/v1"
    assert routes["local-large"] == "https://large.test/v1"
