...
def _valid_model_data() -> dict[str, object]:
    return {
        ...
        "session_summary": "The user requested one small step at a time.",
        "candidate_memories": [
            {
                "candidate_type": "interaction_preference",
                "content": "The user prefers one small step at a time.",
                "source_message_ids": ["msg_005", "msg_005"],
...
class ScriptedClient:
    ...
    async def generate_structured(
        ...
        *,
        ...
        assert output_model is SessionFinalizerResult
...
async def test_unknown_nested_source_uses_fake_fallback(nested_field: str) -> None:
    ...
    if nested_field == "action_items":
        data[nested_field] = [
            {"content": "Write one sentence.", "source_message_ids": ["unknown"]}
        ]
    else:
        candidate = dict(data["candidate_memories"][0])  # type: ignore[index]
        candidate["source_message_ids"] = ["unknown"]
        data[nested_field] = [candidate]
...
async def test_candidate_without_sources_uses_fake_fallback() -> None:
    ...
    data["candidate_memories"] = [candidate]
...
async def test_model_evaluated_outcome_uses_recorded_feedback_and_source() -> None:
    ...
    data["strategy_outcomes"] = [
        {
            ...
            "source_message_ids": ["msg_002", "msg_002"],
        }
    ]
    ...
    assert result.session_summary == data["session_summary"]
    assert result.strategy_outcomes[0].source_message_ids == [MessageId("msg_002")]
...
async def test_assistant_source_cannot_masquerade_as_explicit_user_memory() -> None:
    data = _valid_model_data()
    candidate = dict(data["candidate_memories"][0])  # type: ignore[index]
    candidate["source_message_ids"] = ["msg_004"]
    data["candidate_memories"] = [candidate]
    data["source_message_ids"] = ["msg_004"]
