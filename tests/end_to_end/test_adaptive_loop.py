...
class ClarificationRiskAgent:
    """Risk double that returns a soft clarification route."""
    ...
    async def analyze(self, payload: RiskInput) -> RiskResult:
        ...
        return RiskResult(
            ...
            reason_codes=["soft_risk_fixture"],
            confidence=0.4,
        )
...
async def test_rejection_consumes_pending_intervention_and_switches_strategy() -> None:
    ...
    first_pending = await interventions.get_pending(user_id, session_id)
    ...
    records = await interventions.list_for_session(user_id, session_id)
    ...
    assert await interventions.get_pending(user_id, session_id) == pending_second
...
async def test_topic_switch_or_step_request_without_evaluation_is_absent(
    ...
    evaluated_first = (
        await interventions.list_for_session(user_id, session_id)
    )[0]
    assert evaluated_first.status == InterventionStatus.EVALUATED
    assert evaluated_first.explicit_feedback == "absent"
    assert evaluated_first.objective_progress == "unknown"
...
async def test_ambiguous_feedback_does_not_create_strategy_adjustment(
    ...
    state = await states.get_current(user_id, session_id)
...
@pytest.mark.asyncio
async def test_soft_safety_clarification_does_not_block_dialogue() -> None:
    """Soft risk routes should continue through the normal response pipeline."""

    orchestrator, interventions, _ = _build_adaptive_loop()
    orchestrator._risk_agent = ClarificationRiskAgent()
    session_id = SessionId("session_soft_risk")
    user_id = UserId("user_soft_risk")

    result = await orchestrator.handle_turn(
        user_id=user_id,
        session_id=session_id,
        text="我只是有点累，想聊聊今天发生的事情。",
    )

    assert "立即伤害自己或他人" not in result.response
    assert await interventions.get_pending(user_id, session_id) is not None


@pytest.mark.asyncio
async def test_explicit_crisis_route_still_uses_safety_response() -> None:
    """Clear crisis language should keep the hard safety route."""

    orchestrator, interventions, _ = _build_adaptive_loop()
    session_id = SessionId("session_hard_risk")
    user_id = UserId("user_hard_risk")
    ...
    assert "远离可能伤害自己的物品" in result.response
    assert await interventions.get_pending(user_id, session_id) is None
