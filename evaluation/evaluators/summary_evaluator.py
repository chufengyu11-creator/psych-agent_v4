...
def evaluate_summary_faithfulness(
    ...
    *,
    ...
    previous_sources = (
        set(previous_summary.source_message_ids)
    ...
    if covered_from is not None and covered_to is not None:
        coverage_order_valid = (
            covered_from.sequence_number <= covered_to.sequence_number
        )
    ...
    checks = (
        ...
        (
            ...
            == (
                previous_summary.summary_version + 1
                if previous_summary is not None
                else 1
            ),
        ),
...
def _pending_intervention_has_response(
    ...
    return any(
        ...
        for response in summary.strategy_responses
        for strategy in pending_only
    )
