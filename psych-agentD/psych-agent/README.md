...
Runtime modes, application resource ownership, and the manual model-assisted
SQLite smoke are documented in `docs/runtime_modes.md`.

Redis resource ownership plus `GET /health/live` and `GET /health/ready` are
documented in `docs/redis_and_health.md`. Redis is not yet a task queue and no
worker consumer is implemented.

Copyable Linux environment, systemd, preflight, deployment, and acceptance
assets are documented in `docs/linux_deployment.md`.
