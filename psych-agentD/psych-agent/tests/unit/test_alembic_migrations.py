...
def test_script_directory_has_one_base_and_one_head() -> None:
    """The initial revision should be both the single base and single head."""

    scripts = ScriptDirectory.from_config(_config())
    revision = scripts.get_revision(REVISION)

    assert scripts.get_heads() == [REVISION]
    assert scripts.get_base() == REVISION
...
def test_initial_migration_module_is_importable_and_callable() -> None:
    ...
    assert revision is not None
    assert revision.module.revision == REVISION
...
def test_offline_upgrade_creates_all_tables_jsonb_and_partial_index() -> None:
    ...
    assert (
        "CREATE UNIQUE INDEX uq_intervention_events_session_id_pending "
        "ON intervention_events (session_id) WHERE status = 'pending'"
        in normalized_sql
    )
