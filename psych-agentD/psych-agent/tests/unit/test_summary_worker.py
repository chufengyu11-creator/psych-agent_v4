...
class CountingSummaryRepository(InMemorySummaryRepository):
    ...
    async def save_version(self, summary: RollingSummary) -> None:
        """Count and persist a summary version."""

        self.save_calls += 1
        await super().save_version(summary)
    ...
    async def generate_structured(
        ...
        *,
        ...
        return output_model.model_validate(
            {
                "current_problem": {
                    "value": "model-backed worker summary",
                    "evidence": [
                        {
                            "message_id": "msg_1",
                            "quote": "Please summarize this.",
                        }
                    ],
                }
            }
...
async def test_summary_worker_generates_and_saves_grounded_summary() -> None:
    ...
    first = await messages.create_user_message(session_id, "I feel stuck at work.")
    last = await messages.create_assistant_message(session_id, "What feels most difficult?")
    ...
    assert await summaries.get_current(session_id) == result
...
async def test_summary_worker_increments_version_and_does_not_resave_without_new_messages() -> None:
    ...
    first = await messages.create_user_message(session_id, "First update")
    ...
    second = await messages.create_user_message(session_id, "Second update")
...
async def test_summary_worker_saves_model_backed_summary() -> None:
    ...
    messages = InMemoryMessageRepository()
    summaries = CountingSummaryRepository()
    first = await messages.create_user_message(session_id, "Please summarize this.")
...
async def test_automatic_summary_retains_the_latest_turn() -> None:
    ...
    first_user = await messages.create_user_message(session_id, "First user statement")
    first_assistant = await messages.create_assistant_message(session_id, "First response")
    ...
    retained_user = await messages.create_user_message(session_id, "Retained user statement")
    retained_assistant = await messages.create_assistant_message(
        session_id, "Retained response"
    )
    worker = SummaryWorker(
        message_repository=messages,
        state_repository=InMemoryStateRepository(),
    ...
    result = await worker.handle_summary_requested(
        RollingSummaryRequestedEvent(session_id=session_id, trigger="post_turn")
    )
...
async def test_automatic_summary_skips_first_turn_when_nothing_is_old_enough() -> None:
    ...
    messages = InMemoryMessageRepository()
    summaries = CountingSummaryRepository()
    await messages.create_user_message(session_id, "First user message")
    await messages.create_assistant_message(session_id, "First assistant message")
...
async def test_automatic_summary_advances_by_one_old_turn() -> None:
    ...
    summaries = CountingSummaryRepository()
    worker = make_worker(messages, summaries)
    first_user = await messages.create_user_message(session_id, "u1")
    first_assistant = await messages.create_assistant_message(session_id, "a1")
    await messages.create_user_message(session_id, "u2")
    second_assistant = await messages.create_assistant_message(session_id, "a2")

    first_summary = await worker.handle_summary_requested(
        RollingSummaryRequestedEvent(session_id=session_id, trigger="post_turn")
    )
    await messages.create_user_message(session_id, "u3")
    third_assistant = await messages.create_assistant_message(session_id, "a3")
...
async def test_explicit_boundary_does_not_retain_recent_messages() -> None:
    ...
    summaries = CountingSummaryRepository()
    first_user = await messages.create_user_message(session_id, "u1")
    await messages.create_assistant_message(session_id, "a1")
    await messages.create_user_message(session_id, "u2")
    boundary = await messages.create_assistant_message(session_id, "a2")
...
async def test_session_close_without_boundary_covers_the_remaining_messages() -> None:
    ...
    summaries = CountingSummaryRepository()
    first_user = await messages.create_user_message(session_id, "u1")
    final_assistant = await messages.create_assistant_message(session_id, "a1")

    result = await make_worker(messages, summaries).handle_summary_requested(
        RollingSummaryRequestedEvent(session_id=session_id, trigger="session_close")
    )
...
async def test_retained_state_source_does_not_leak_into_summary() -> None:
    ...
    first_user = await messages.create_user_message(session_id, "older grounded topic")
    await messages.create_assistant_message(session_id, "older assistant response")
    retained_user = await messages.create_user_message(session_id, "retained secret topic")
    await messages.create_assistant_message(session_id, "retained assistant response")
...
async def test_retained_pending_intervention_does_not_leak_into_summary() -> None:
    ...
    summaries = CountingSummaryRepository()
    interventions = InMemoryInterventionRepository()
    await messages.create_user_message(session_id, "older user message")
    await messages.create_assistant_message(session_id, "older assistant message")
    await messages.create_user_message(session_id, "retained user message")
    retained_assistant = await messages.create_assistant_message(
        session_id, "retained assistant message"
    )
    await interventions.create_pending(
        session_id,
