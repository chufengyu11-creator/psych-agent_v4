...
def test_message_constraints_and_foreign_keys_are_exact() -> None:
    ...
    assert _column_names(
        unique_constraints["uq_messages_session_id_sequence_number"]
    ) == ("session_id", "sequence_number")
    assert _foreign_key_contracts(table) == {
        "fk_messages_session_id_sessions": ("sessions.id", "RESTRICT"),
...
def test_message_indexes_have_expected_names_and_column_order() -> None:
    ...
    assert _column_names(indexes["ix_messages_session_id"]) == ("session_id",)
    assert _column_names(indexes["ix_messages_session_id_created_at"]) == (
        "session_id",
        "created_at",
    )
...
def test_session_state_constraints_foreign_keys_and_indexes_are_exact() -> None:
    ...
    assert _column_names(
        unique_constraints["uq_session_state_versions_session_id_version"]
    ) == ("session_id", "version")
    assert _foreign_key_contracts(table) == {
    ...
    assert _column_names(
        indexes["ix_session_state_versions_session_id_version"]
    ) == ("session_id", "version")
...
def test_intervention_indexes_include_cross_dialect_pending_uniqueness() -> None:
    ...
    assert _column_names(
        indexes["ix_intervention_events_session_id_status_created_at"]
    ) == ("session_id", "status", "created_at")

    partial_index = indexes["uq_intervention_events_session_id_pending"]
    assert partial_index.unique
    assert _column_names(partial_index) == ("session_id",)
...
def test_models_and_partial_index_compile_to_postgresql_ddl() -> None:
    ...
    assert "FOREIGN KEY(session_id)" in message_ddl
    ...
    assert "uq_intervention_events_session_id_pending" in partial_index_ddl
    assert "ON intervention_events (session_id)" in partial_index_ddl
