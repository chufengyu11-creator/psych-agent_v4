...
async def test_inline_post_turn_queue_retains_latest_turn_before_summarizing() -> None:
    ...
    first_assistant = await messages.create_assistant_message(
        session_id, "First assistant message"
    )
    ...
    await messages.create_user_message(session_id, "Second user message")
    await messages.create_assistant_message(session_id, "Second assistant message")
    await queue.enqueue("post_turn", str(user_id), str(session_id))

    current = await summaries.get_current(session_id)
