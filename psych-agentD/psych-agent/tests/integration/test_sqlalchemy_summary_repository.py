...
async def test_sqlalchemy_summary_repository_saves_and_reads_current_summary(
    ...
    async with session_factory() as session:
        ...
        first = await message_repository.create_user_message(session_id, "first user message")
        ...
        await repository.save_version(summary)
        await session.commit()

    async with session_factory() as session:
        repository = SqlAlchemySummaryRepository(session)
        current = await repository.get_current(session_id)
