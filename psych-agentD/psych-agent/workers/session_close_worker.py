...
class SessionCloseMemoryWorker:
    ...
    async def handle_session_close_requested(
        ...
        await self._session_repository.assert_owned_by(event.session_id, event.user_id)
        messages = await self._message_repository.get_recent(
            event.session_id,
            limit=self._message_limit,
        )
        final_state = await self._state_repository.get_current(event.session_id)
        interventions = await self._intervention_repository.list_for_session(event.session_id)
        rolling_summary = await self._summary_repository.get_current(event.session_id)
        ...
        memory_write_results = await self._memory_worker.handle_memory_candidate_created(
        ...
        await self._session_repository.close(event.session_id, event.user_id)
