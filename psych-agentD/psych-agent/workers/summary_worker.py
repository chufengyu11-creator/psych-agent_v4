...
class SummaryWorker:
    ...
    async def handle_summary_requested(
        ...
        previous_summary = await self._summary_repository.get_current(event.session_id)
        ...
        if event.after_message_id is not None:
            after_message = await self._message_repository.get_by_id(event.after_message_id)
        ...
        if previous_summary is not None:
            covered_to = await self._message_repository.get_by_id(previous_summary.covered_to)
        ...
        current_state = _filter_state_for_coverage(
            await self._state_repository.get_current(event.session_id),
        ...
        await self._summary_repository.save_version(summary)
        return summary
