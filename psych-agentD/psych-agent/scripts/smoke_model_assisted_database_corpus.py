...
async def run_business_flow(
    ...
    async with transactional_session(session_factory) as session:
        ...
        summary_repository = SqlAlchemySummaryRepository(session)
        intervention_repository = SqlAlchemyInterventionRepository(session)
        recent_messages = await message_repository.get_recent(session_id, limit=20)
        current_state = await state_repository.get_current(session_id)
        previous_summary = await summary_repository.get_current(session_id)
        interventions = await intervention_repository.list_for_session(session_id)
        ...
        await summary_repository.save_version(summary)
