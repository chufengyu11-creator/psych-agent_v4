...
class RuntimeSmokeSummary:
    ...
    fake_components_valid: bool = False
...
async def run_runtime_smoke(
    ...
    *,
    ...
    def observe_client(
        ...
        http_client: OpenAICompatibleHTTPClient,
    ...
    try:
        ...
        summary.fake_components_valid = all(
            (
                isinstance(runtime.response_agent, FakeResponseAgent),
                isinstance(runtime.output_guard, FakeOutputGuard),
                isinstance(runtime.feedback_evaluator, FakeFeedbackEvaluator),
            )
        )
    ...
    async with session_factory() as session:
        ...
        summary.sessions_rows = int(
            await session.scalar(
                ...
                .where(SessionModel.id == summary.session_id)
            )
            or 0
        )
        messages = list(
            (
                await session.scalars(
                    select(MessageModel)
                    .where(MessageModel.session_id == summary.session_id)
        ...
        state_rows = list(
            (
                await session.scalars(
                    ...
                    .where(SessionStateVersionModel.session_id == summary.session_id)
        ...
        summary.intervention_events_rows = int(
            await session.scalar(
                ...
                .where(InterventionEventModel.session_id == summary.session_id)
            )
            or 0
        )
        session_row = await session.get(SessionModel, summary.session_id)
        summary.session_status = session_row.status if session_row is not None else None
...
def _aggregate_agent_stats(
    ...
    for agent_name in ("risk_agent", "state_tracker", "strategy_planner"):
        agent_observations = [
...
def _acceptance_matches(summary: RuntimeSmokeSummary) -> bool:
    """Require two successful real structured calls per Agent and DB contract."""
    ...
    return all(
        (
            ...
            summary.fake_components_valid,
            summary.structured_calls == 6,
            summary.fallback_count == 0,
            all(
                item.calls == 2
                and item.api_success == 2
                and item.schema_success == 2
                and item.effective_result_available == 2
