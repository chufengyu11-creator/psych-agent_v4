...
class InMemorySummaryRepository:
    ...
    def __init__(self) -> None:
        ...
        self._summaries: dict[SessionId, RollingSummary] = {}
...
class SqlAlchemySummaryRepository:
    ...
    async def get_current(self, session_id: SessionId) -> RollingSummary | None:
        ...
        if session_row.current_summary_version == 0:
            return None
        latest = await self._latest_summary_row(session_id)
        if latest is None:
            return None
    ...
    async def save_version(self, summary: RollingSummary) -> None:
        ...
        session_row = await self._session.get(SessionModel, str(summary.session_id))
        if session_row is None:
            raise SessionNotFoundError(
                f"Session {str(summary.session_id)!r} does not exist."
            )
        previous_row = await self._latest_summary_row(summary.session_id)
